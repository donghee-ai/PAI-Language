# Language ↔ Coordinator 명령 스키마 (초안 v0.4)

> **상태**: 초안 — Action팀 / Coordinator팀 리뷰 및 합의 필요
> **작성일**: 2026-05-07 (v0.1) / 2026-05-09 (v0.2) / 2026-05-11 (v0.3) / 2026-05-12 (v0.4)
> **데모 시나리오**: "스포츠볼 집어서 그릇에 넣어줘" — `target="sports ball"`, `destination="bowl"`. 초기 `ball`/`basket` 안은 COCO 사전학습 YOLO에 `basket`이 없어 폐기.
> **라벨**: `target` / `destination` 은 `shared/constants.py` `COCO_LABELS`(COCO 80개) 중 하나 또는 `"none"`. 커스텀 객체는 Vision fine-tuning / open-vocab 도입 후 확장.
> **작성**: Language 파트
...
```json
{
  ...
  "data": {
    ...
    "target": "sports ball",
    "destination": "bowl",
    "reasoning": "사용자가 공을 그릇에 담으라고 요청함",
    "raw_input": "공 잡아서 그릇에 넣어줘",
...
| `target` | string | Y | 조작 대상 객체의 YOLO label (`COCO_LABELS` 중 하나). 대상 없으면 `"none"` |
| `destination` | string | Y | 목적지 객체의 YOLO label (`COCO_LABELS` 중 하나). 해당 없으면 `"none"` |
...
### 시나리오 1 — 스포츠볼 집어서 그릇에 넣기 (메인 데모)

```
User: "공 집어서 그릇에 넣어줘"

Vision 상태: sports ball(감지됨), bowl(감지됨)
...
→ robot_command:
{
  ...
  "target": "sports ball",
  "destination": "bowl",
  "reasoning": "공을 그릇에 담는 복합 동작 요청",
  "raw_input": "공 집어서 그릇에 넣어줘",
  "vision_confirmed": true
}
```
...
| 항목 | 현재 초안 | 확인 필요 내용 |
|------|-----------|----------------|
| `target` / `destination` 값 | `COCO_LABELS`(COCO 80개) 문자열 그대로. 데모는 `sports ball` / `bowl` | Action 팀이 label 기반 객체 탐색 가능한지. 커스텀 객체 필요 시 Vision fine-tuning 일정 |
...
| v0.4 | 2026-05-12 | 라벨 집합을 COCO 80개(`shared/constants.py` `COCO_LABELS`)로 임시 확정. 데모 시나리오를 "스포츠볼 집어서 그릇에 넣기"(`sports ball` / `bowl`)로 변경 — COCO에 `basket`이 없음. 예시·시나리오·필드 설명 일괄 갱신 |
