...
6. target / destination 은 위 '인식 가능한 객체 라벨' 목록의 문자열을 그대로 쓴다. 목록에 없는 문자열은 절대 만들지 않는다.
7. 카메라에 해당 객체가 없으면 가장 합리적인 판단을 하되, 반드시 command.reasoning에 명시
8. 명령 의도가 모호하거나 위험하면 command=null로 두고 answer에 다시 물어보는 멘트를 한다.
